...
router.get("/overview",asyncHandler(async(req,res)=>{
  ...
  const org=req.user.organization_id,status=String(req.query.status||""),search=String(req.query.search||"").trim(),mine=req.query.mine==="true";
  ...
  const result=await withTenantTransaction(org,async client=>{
    const params=[org],where=["c.organization_id=$1"];
    if(!(req.user.permissions||[]).includes("documents.restricted.read"))where.push("c.confidentiality<>'restricted'");
    if(status){params.push(status);where.push(`c.status=$${params.length}`)}
    if(search){params.push(`%${search}%`);where.push(`(c.registration_no ILIKE $${params.length} OR c.subject ILIKE $${params.length} OR c.requester_name ILIKE $${params.length})`)}
    if(mine){params.push(req.user.id);where.push(`c.assigned_user_id=$${params.length}`)}
    const counts=await client.query(`SELECT count(*)::int total,
      ...
      count(*) FILTER(WHERE due_date<CURRENT_DATE AND status NOT IN('response_sent','closed','rejected','cancelled'))::int overdue,
      count(*) FILTER(WHERE status='response_sent')::int responded,
      count(*) FILTER(WHERE status='closed')::int closed FROM complaint_cases WHERE organization_id=$1`,[org]);
    const total=(await client.query(`SELECT count(*)::int value FROM complaint_cases c WHERE ${where.join(" AND ")}`,params)).rows[0].value;
    const listParams=[...params,pageSize,offset];
    const items=await client.query(`SELECT c.*,u.full_name assigned_name,d.name assigned_unit_name,
      (c.due_date<CURRENT_DATE AND c.status NOT IN('response_sent','closed','rejected','cancelled')) overdue
      ...
      WHERE ${where.join(" AND ")} ORDER BY c.created_at DESC LIMIT $${params.length+1} OFFSET $${params.length+2}`,listParams);
    return {summary:counts.rows[0],items:items.rows,pagination:{page,pageSize,total}};
  });
  res.json(result);
}));

router.get("/:id",asyncHandler(async(req,res)=>{
  if(denyRead(req,res))return;const id=z.string().uuid().safeParse(req.params.id);if(!id.success)return res.status(400).json({error:"INVALID_ID"});
  const result=await withTenantTransaction(req.user.organization_id,async client=>{
    const item=await client.query(`SELECT c.*,u.full_name assigned_name,d.name assigned_unit_name FROM complaint_cases c
      LEFT JOIN users u ON u.organization_id=c.organization_id AND u.id=c.assigned_user_id
      LEFT JOIN departments d ON d.organization_id=c.organization_id AND d.id=c.assigned_unit_id
      WHERE c.organization_id=$1 AND c.id=$2`,[req.user.organization_id,id.data]);
    if(!item.rowCount)return null;
    if(item.rows[0].confidentiality==="restricted"&&!(req.user.permissions||[]).includes("documents.restricted.read"))throw Object.assign(new Error("RESTRICTED_DOCUMENT_FORBIDDEN"),{status:403});
    const [events,workflow,documents]=await Promise.all([
      client.query(`SELECT e.*,u.full_name actor_name FROM complaint_case_events e JOIN users u ON u.organization_id=e.organization_id AND u.id=e.actor_user_id WHERE e.organization_id=$1 AND e.complaint_case_id=$2 ORDER BY e.created_at,e.id`,[req.user.organization_id,id.data]),
      client.query(`SELECT e.*,u.full_name actor_name FROM workflow_transition_events e JOIN workflow_cases w ON w.organization_id=e.organization_id AND w.id=e.case_id JOIN users u ON u.organization_id=e.organization_id AND u.id=e.actor_user_id WHERE w.organization_id=$1 AND w.subject_type='complaint_case' AND w.subject_id=$2 ORDER BY e.created_at,e.id`,[req.user.organization_id,id.data]),
      client.query(`SELECT d.id,d.document_no,d.title,d.classification_code AS confidentiality,d.status FROM document_links l JOIN documents d ON d.organization_id=l.organization_id AND d.id=l.document_id WHERE l.organization_id=$1 AND l.entity_type='complaint_case' AND l.entity_id=$2`,[req.user.organization_id,id.data]),
    ]);
    const visibleDocuments=(req.user.permissions||[]).includes("documents.restricted.read")?documents.rows:documents.rows.filter(x=>x.confidentiality!=="restricted");
    return {item:item.rows[0],events:events.rows,workflow:workflow.rows,documents:visibleDocuments};
  });
...
router.post("/:id/commands/:command",asyncHandler(async(req,res)=>{
  ...
  const set={};if(v.resolution!==undefined)set.resolution=v.resolution;if(v.responseDocumentId)set.response_document_id=v.responseDocumentId;if(v.closeReason!==undefined)set.close_reason=v.closeReason;if(req.params.command==="close")set.closed_at=new Date();
