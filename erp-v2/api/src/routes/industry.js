...
router.post("/work-types",manage,asyncHandler(async(req,res)=>{const parsed=z.object({code,name:text(200),category:text(150),description:optionalText(2000),operationalStream:z.enum(["core_service","internal_operation"]).nullable().optional()}).safeParse(req.body);if(!parsed.success)return res.status(400).json({error:"Ажлын төрлийн мэдээлэл буруу байна"});const v=parsed.data,result=await getPool().query(`INSERT INTO organization_work_types(organization_id,code,name,category,description,operational_stream) VALUES($1,$2,$3,$4,$5,$6) RETURNING *`,[req.user.organization_id,v.code,v.name,v.category,v.description,v.operationalStream||null]);await writeAudit(req,"industry.work_type_create","work_type",result.rows[0].id,v);res.status(201).json({item:result.rows[0]})}));
