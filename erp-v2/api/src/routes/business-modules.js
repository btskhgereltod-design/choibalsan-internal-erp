...
const { authenticate, authenticatePlatform, requireRoles, requireModule, requireSystemRoles } = require("../middleware/auth");
...
tenantRouter.get("/inventory", asyncHandler(async(req,res)=>{
  ...
  res.json({warehouses:warehouses.rows,items:items.rows,movements:movements.rows});
}));
