...
tenantRouter.patch("/settings", requireSystemRoles("owner"), asyncHandler(async(req,res)=>{const parsed=z.object({name:text(200),shortName:emptyText(100),logoUrl:z.string().trim().max(1000).default(""),homeBannerUrl:z.string().trim().max(1000).optional(),homeWelcomeText:z.string().trim().max(300).optional(),primaryColor:z.string().regex(/^#[0-9a-fA-F]{6}$/),accentColor:z.string().regex(/^#[0-9a-fA-F]{6}$/),address:emptyText(500),phone:emptyText(100),email:z.union([z.string().trim().email(),z.literal("")]),registrationNo:emptyText(100)}).safeParse(req.body);if(!parsed.success)return res.status(400).json({error:"Тохиргооны мэдээлэл буруу байна"});const v=parsed.data,client=await getPool().connect();try{await client.query("BEGIN");await client.query(`UPDATE organizations SET name=$2,updated_at=now() WHERE id=$1`,[req.user.organization_id,v.name]);const result=await client.query(`UPDATE organization_settings SET short_name=$2,logo_url=$3,home_banner_url=COALESCE($4,home_banner_url),home_welcome_text=COALESCE($5,home_welcome_text),primary_color=$6,accent_color=$7,address=$8,phone=$9,email=$10,registration_no=$11,updated_by=$12,updated_at=now() WHERE organization_id=$1 RETURNING *`,[req.user.organization_id,v.shortName,v.logoUrl,v.homeBannerUrl??null,v.homeWelcomeText??null,v.primaryColor,v.accentColor,v.address,v.phone,v.email,v.registrationNo,req.user.id]);await client.query("COMMIT");await writeAudit(req,"organization.settings","organization",req.user.organization_id,{name:v.name,primaryColor:v.primaryColor,homeBrandingChanged:Object.hasOwn(v,"homeBannerUrl")||Object.hasOwn(v,"homeWelcomeText")});res.json({item:result.rows[0]});}catch(error){await client.query("ROLLBACK").catch(()=>{});throw error}finally{client.release()}}));
