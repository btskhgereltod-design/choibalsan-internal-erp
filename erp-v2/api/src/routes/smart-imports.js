...
router.post("/",upload.single("file"),asyncHandler(async(req,res)=>{
  ...
  try{await client.query("BEGIN");const context=await importContext(client,org);const rows=validateRows(parsed.rows,mapping,context),summary={...summarize(rows,context),truncated:parsed.truncated,sourceSheet:parsed.sheetName,compatibilityNormalized:parsed.compatibilityNormalized,aiWarnings:[...(ai?.warnings||[]),...(aiWarning?[aiWarning]:[])]};
