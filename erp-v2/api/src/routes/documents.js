...
router.get("/",asyncHandler(async(req,res)=>{
  ...
  const result=await getPool().query(`SELECT d.*,v.version_no,v.original_name,v.mime_type,v.size_bytes,v.content_sha256
    FROM documents d LEFT JOIN document_versions v ON v.organization_id=d.organization_id AND v.id=d.current_version_id
    WHERE d.organization_id=$1 AND ($2::text IS NULL OR d.status=$2) ORDER BY d.updated_at DESC LIMIT 250`,[req.user.organization_id,status.data||null]);
...
router.get("/:id/versions",asyncHandler(async(req,res)=>{
  ...
  const result=await getPool().query(`SELECT id,version_no,original_name,mime_type,size_bytes,content_sha256,metadata,created_by,created_at
    FROM document_versions WHERE organization_id=$1 AND document_id=$2 ORDER BY version_no DESC`,[req.user.organization_id,id.data]);
  res.json({items:result.rows});
}));
...
router.post("/:id/versions",uploadOne,asyncHandler(async(req,res)=>{
  ...
  const hash=crypto.createHash("sha256").update(req.file.buffer).digest("hex"),storageKey=crypto.randomUUID(),filePath=path.join(uploadDirectory,storageKey);
  await fs.promises.writeFile(filePath,req.file.buffer,{flag:"wx"});
  const client=await getPool().connect();let item;
  try{await client.query("BEGIN");const doc=await client.query(`SELECT id,status FROM documents WHERE organization_id=$1 AND id=$2 FOR UPDATE`,[req.user.organization_id,id.data]);
...
router.get("/:documentId/versions/:versionId/file",asyncHandler(async(req,res,next)=>{
  ...
  const result=await getPool().query(`SELECT original_name,storage_key,mime_type FROM document_versions WHERE organization_id=$1 AND document_id=$2 AND id=$3`,[req.user.organization_id,documentId.data,versionId.data]);
...
router.post("/:id/transitions",asyncHandler(async(req,res)=>{
  ...
  try{await client.query("BEGIN");const current=await client.query(`SELECT * FROM documents WHERE organization_id=$1 AND id=$2 FOR UPDATE`,[req.user.organization_id,id.data]);if(!current.rowCount){await client.query("ROLLBACK");return res.status(404).json({error:"Баримт олдсонгүй"})}
