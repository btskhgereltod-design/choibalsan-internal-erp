...
async function main(){
  ...
  try{
    ...
    for(const row of data.faults||[]){if(await linked("sl_faults",row.id)){counts.skipped++;continue}const key=`${legacyAssetTable(row.location_type)}:${row.location_id}`,assetId=assetIds.get(key)||null,affected=Math.max(0,integer(row.broken_count)),resolved=Math.min(affected,Math.max(0,integer(row.fixed_count))),status=faultStatus(row.status);
      const inserted=await client.query(`INSERT INTO operational_incidents(organization_id,domain,operational_object_id,incident_type,title,location,affected_quantity,resolved_quantity,status,reported_at,reported_by,detail,source_system,external_id)
        VALUES($1,'lighting',$2,$3,$4,$5,$6,$7,$8,COALESCE($9::timestamptz,now()),$10,$11::jsonb,$12,$13) RETURNING id`,[org,assetId,clean(row.category)||"fault",`${clean(row.category)||"Гэмтэл"} — ${clean(row.location_name)||"Байршил"}`,clean(row.location_name),affected,resolved,status,timestamp(row.report_date||row.created_at),users.get(integer(row.reported_by))||null,JSON.stringify({legacyStatus:row.status,totalHeads:row.total_heads,notes:row.notes||""}),data.sourceSystem,String(row.id)]);
    ...
    for(const row of data.work||[]){if(await linked("asset_events",row.id)){counts.skipped++;continue}const assetId=assetIds.get(row.sl_point_id?`sl_points:${row.sl_point_id}`:`sl_ger_inventory:${row.ger_inventory_id}`)||null,status=workStatus(row.status),warnings=[];let due=timestamp(row.end_date||row.work_date);const start=timestamp(row.start_date||row.work_date);if(due&&start&&new Date(due)<new Date(start)){warnings.push("End date precedes start date; due date omitted");due=null}if(Number(row.progress)!==0&&Number(row.progress)!==100)warnings.push("Legacy manual progress preserved only in provenance");
      const inserted=await client.query(`INSERT INTO work_orders(organization_id,operational_object_id,work_type_id,department_id,workflow_policy_id,workflow_stage,title,description,category,priority,status,assigned_to,due_at,created_at,updated_at)
        VALUES($1,$2,$3,$4,$5,$6,$7,$8,'lighting.repair','normal',$9,$10,$11,COALESCE($12::timestamptz,now()),COALESCE($13::timestamptz,now())) RETURNING id`,[org,assetId,route.work_type_id||null,route.organization_unit_id||null,route.workflow_policy_id||null,status==='completed'?'completed':route.workflow_policy_id?'awaiting_safety_start':null,clean(row.title)||`Legacy lighting work ${row.id}`,clean(row.description||row.location),status,users.get(integer(row.assigned_to))||null,due,timestamp(row.created_at),timestamp(row.updated_at)]);
      const id=inserted.rows[0].id;await client.query(`INSERT INTO work_order_events(organization_id,work_order_id,actor_user_id,event_type,to_status,note,detail,created_at) VALUES($1,$2,$3,'created',$4,$5,$6::jsonb,COALESCE($7::timestamptz,now()))`,[org,id,users.get(integer(row.created_by))||null,status,"Хуучин системээс нотолгоотой импортлов",JSON.stringify({source:data.sourceSystem,legacyId:row.id,legacyStatus:row.status}),timestamp(row.created_at)]);await provenance("asset_events",row,"work_order",id,warnings);counts.workOrders++;counts.warnings.push(...warnings.map(x=>`Work ${row.id}: ${x}`));
