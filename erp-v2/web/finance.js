...
function assetsView(d) {
  return `<section class="finance-panel"><h2>Үндсэн хөрөнгийн лавлагаа</h2><p class="finance-note warning">Энд OVERVA-ийн хөрөнгийн мастер бүртгэлийг харуулж байна. Хуучин ERP-ийн өртөг, элэгдэл, үлдэгдэл өртгийг баталгаажуулалгүй хуулсангүй.</p><div class="module-table-wrap"><table class="module-table"><thead><tr><th>Код</th><th>Нэр</th><th>Ангилал</th><th>Төлөв</th></tr></thead><tbody>${d.assets.map((x) => `<tr><td>${esc(x.code)}</td><td>${esc(x.name)}</td><td>${esc(x.category)}</td><td>${esc(x.status)}</td></tr>`).join("") || `<tr><td colspan="4">Хөрөнгө бүртгэгдээгүй байна.</td></tr>`}</tbody></table></div></section>`;
}
function toolsView(d) {
  return `<div class="finance-layout"><section class="finance-panel"><h2>Орлого, зарлагын чиг хандлага</h2>${financeChart(d.trend)}</section><section class="finance-panel"><h2>Баталгаатай импорт</h2><form class="finance-form" id="financeImportForm"><input name="file" type="file" accept=".csv,.xlsx" required><button class="primary">Импортлох</button><button class="secondary" type="button" id="financeTemplate">Загвар CSV татах</button></form><small>Хуучин ERP-ийн бодит дүнг энд автоматаар хуулалгүй, эхлээд шалгаж импортлоно.</small></section></div>`;
}
