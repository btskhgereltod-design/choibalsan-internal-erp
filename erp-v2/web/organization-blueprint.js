...
state.organizationBlueprint={catalog:null,latest:null,draft:null,method:null,interview:null,pendingAnswer:null,loading:false,busy:false,advanced:false};
...
async function loadOrganizationBlueprint(){
  ...
  try{const [data,method,latestInterview]=await Promise.all([api("/api/organization-blueprints/catalog"),api("/api/organization-blueprints/method"),api("/api/organization-blueprints/interviews/latest")]);b.catalog=data;b.latest=data.latest;b.draft=data.latest?.status==="draft"?data.latest:null;b.method=method;if(latestInterview.item)await refreshBlueprintInterview(latestInterview.item.id);}
