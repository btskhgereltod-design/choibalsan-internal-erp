//+------------------------------------------------------------------+
//|                              Gold_Dual_Trend_Pyramid_EA.mq4      |
//|  Runs independent BUY and SELL trend pyramids at the same time.  |
//+------------------------------------------------------------------+
#property strict
...
input int BuyMagicNumber = 20260821;
input int SellMagicNumber = 20260822;
...
int OnInit()
{
   ...
   AdoptSide(1);
   AdoptSide(-1);
   Print("Gold Dual Trend Pyramid EA loaded on ", Symbol());
...
void OnDeinit(const int reason)
{
   if(ShowChartStatus)
      Comment("");
   Print("Gold Dual Trend Pyramid EA stopped. Reason: ", reason);
}
...
void ShowStatus()
{
   ...
   Comment("GOLD DUAL TREND PYRAMID\n",
