//+------------------------------------------------------------------+
//|                       Gold_H1_Breakout_Trend_Pyramid_EA.mq4      |
//|  Starts from an H1-open breakout and adds only into profit.      |
//+------------------------------------------------------------------+
#property strict

input string BreakoutSettings = "--- Initial breakout ---";
input ENUM_TIMEFRAMES BreakoutTimeframe = PERIOD_H1;
input double InitialBreakoutPriceDistance = 10.0;
...
input int MagicNumber = 20260820;
...
int OnInit()
{
   if(InitialBreakoutPriceDistance <= 0.0 || StartLot <= 0.0 ||
      LotMultiplier < 1.0 || MaxOrders < 1 ||
   ...
   if(count > 0)
   {
      ...
      g_lastSignalBarTime = iTime(Symbol(), BreakoutTimeframe, 0);

   Print("Gold H1 Breakout Trend Pyramid EA loaded on ", Symbol());
...
void OnDeinit(const int reason)
{
   if(ShowChartStatus)
      Comment("");
   Print("Gold H1 Breakout Trend Pyramid EA stopped. Reason: ", reason);
}
...
void OnTick()
{
   ...
   if(count == 0)
   {
      ...
      if(!AllowNewCycles)
      {
         ShowStatus(0, 0, 0.0, "New cycles disabled.");
         return;
      }
      ...
      if(!UpdateBreakoutReference())
      {
         ShowStatus(0, 0, 0.0, "Waiting for next H1 reference.");
         return;
      }

      int signal = GetBreakoutDirection();
      if(signal == 0)
      {
         ShowStatus(signal, 0, 0.0, "Waiting for H1 open breakout.");
         return;
      }
...
void FinishBasket()
{
   ...
   g_peakBasketCount = 0;
   g_lastSignalBarTime = iTime(Symbol(), BreakoutTimeframe, 0);
   g_referencePrice = iOpen(Symbol(), BreakoutTimeframe, 0);
   g_breakoutArmed = false;
}
...
{
   if(!g_breakoutArmed || g_referencePrice <= 0.0)
      return(0);

   RefreshRates();
   if(Ask >= g_referencePrice + InitialBreakoutPriceDistance)
      return(1);
   if(Bid <= g_referencePrice - InitialBreakoutPriceDistance)
      return(-1);
   return(0);
}

//+------------------------------------------------------------------+
bool UpdateBreakoutReference()
{
   datetime currentBar = iTime(Symbol(), BreakoutTimeframe, 0);
   if(currentBar <= 0)
      return(false);
   ...
   if(g_lastSignalBarTime == 0)
   {
      ...
      g_referencePrice = iOpen(Symbol(), BreakoutTimeframe, 0);
      g_breakoutArmed = StartFirstCycleImmediately;
      return(g_breakoutArmed);
   }
                ...
                const string detail)
{
   ...
   Comment("GOLD H1 BREAKOUT TREND PYRAMID\n",
           "Direction: ", directionText, "  Level: ", count, "/", MaxOrders, "\n",
           "H1 reference: ", DoubleToString(g_referencePrice, Digits),
           "  Breakout: +/-", DoubleToString(InitialBreakoutPriceDistance, 2), "\n",
