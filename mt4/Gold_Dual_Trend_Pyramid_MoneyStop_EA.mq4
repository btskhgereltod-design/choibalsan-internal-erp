//+------------------------------------------------------------------+
//|                    Gold_Dual_Trend_Pyramid_MoneyStop_EA.mq4      |
...
input double BasketStopMoney = 20.0;
input double BrokerEmergencyStopMoney = 25.0;
...
input int BuyMagicNumber = 20260823;
input int SellMagicNumber = 20260824;
...
int OnInit()
{
   ...
   AdoptSide(1);
   AdoptSide(-1);
   Print("Gold Dual Trend Pyramid Money Stop EA loaded on ", Symbol());
...
void OnDeinit(const int reason)
{
   if(ShowChartStatus)
      Comment("");
   Print("Gold Dual Trend Pyramid Money Stop EA stopped. Reason: ", reason);
}
...
int OpenSideOrder(const int direction, const int level)
{
   ...
   string comment = "GOLD_DUAL_" + side + "_L" + IntegerToString(level);
   ResetLastError();
