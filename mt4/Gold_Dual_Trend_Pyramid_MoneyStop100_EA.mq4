//+------------------------------------------------------------------+
//|                 Gold_Dual_Trend_Pyramid_MoneyStop100_EA.mq4     |
//|  Dual pyramids with independent account-currency basket stops.   |
//+------------------------------------------------------------------+
#property strict
...
input double BasketStopMoney = 100.0;
input double BrokerEmergencyStopMoney = 125.0;
...
input int BuyMagicNumber = 20260825;
input int SellMagicNumber = 20260826;
...
int OnInit()
{
   if(StartLot <= 0.0 || LotMultiplier < 1.0 || MaxOrdersPerSide < 1 ||
      AddStepPriceDistance <= 0.0 || BasketStopMoney <= 0.0 ||
      BrokerEmergencyStopMoney < BasketStopMoney ||
   ...
   AdoptSide(1);
   AdoptSide(-1);
   Print("Gold Dual Trend Pyramid Money Stop 100 EA loaded on ", Symbol());
...
void OnDeinit(const int reason)
{
   if(ShowChartStatus)
      Comment("");
   Print("Gold Dual Trend Pyramid Money Stop 100 EA stopped. Reason: ", reason);
}
...
int OpenSideOrder(const int direction, const int level)
{
   ...
   if(level == 1 && valuePerPrice > 0.0)
      stopPrice = price - direction * BrokerEmergencyStopMoney / valuePerPrice;
   ...
   string comment = "GOLD_DUAL100_" + side + "_L" + IntegerToString(level);
   ResetLastError();
...
{
   double commonStop = CalculateMoneyStopPrice(direction, BrokerEmergencyStopMoney);
   if(commonStop <= 0.0)
   {
...
void ShowStatus()
{
   ...
   Comment("GOLD DUAL TREND PYRAMID - MONEY STOP\n",
           ...
           "  Money stop/side: -", DoubleToString(BasketStopMoney, 2),
