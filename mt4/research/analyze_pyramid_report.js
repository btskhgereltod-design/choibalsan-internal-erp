...
const reportPath = process.argv[2] || path.join(__dirname, 'Trend_Pyramid_Report.htm');
...
for (const event of events) {
  ...
  if (event.type === 'buy' || event.type === 'sell') {
    if (!currentBasket) currentBasket = beginBasket(event);
    const level = currentBasket.trades.length + 1;
    const trade = {
    ...
    currentBasket.trades.push(trade);
    currentBasket.maxLevel = Math.max(currentBasket.maxLevel, level);
    activeOrders.set(event.order, trade);
    continue;
  }

  const trade = activeOrders.get(event.order);
  if (!trade || !currentBasket) continue;
  ...
  currentBasket.profit += event.profit;
  currentBasket.endingBalance = event.balance;
  activeOrders.delete(event.order);

  if (activeOrders.size === 0) {
    currentBasket.endTime = event.time;
    const closeTypes = new Set(currentBasket.trades.map((item) => item.closeType));
    if (closeTypes.has('t/p')) currentBasket.exitType = 'final_target';
    else if (closeTypes.has('close at stop')) currentBasket.exitType = 'test_end';
    else if (closeTypes.has('close')) currentBasket.exitType = 'manual_or_ea_close';
    else currentBasket.exitType = 'basket_stop';
    currentBasket.profit = Number(currentBasket.profit.toFixed(2));
